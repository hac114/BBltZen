...
namespace BBltZen.Controllers
{
    ...
    public class SessioniQrController : SecureBaseController
    {
        ...
        [HttpGet("{id}")]
        [AllowAnonymous] // ✅ PERMETTE ACCESSO PER SCANSIONE QR
        public async Task<ActionResult<SessioniQrDTO>> GetById(Guid id)
        {
            try
            {
                ...
                if (sessione == null)
                    return SafeNotFound<SessioniQrDTO>("Sessione QR");
        ...
        public async Task<ActionResult<SessioniQrDTO>> GeneraSessioneQr(int tavoloId, [FromQuery] string frontendUrl = "https://bbzen.it")
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(frontendUrl))
                    return SafeBadRequest<SessioniQrDTO>("URL frontend non valido");
                ...
                LogSecurityEvent("QRSessionGenerated", new
                {
                    ...
                    TavoloId = tavoloId,
                    CodiceSessione = sessione.CodiceSessione,
        ...
        public async Task<ActionResult<SessioniQrDTO>> Create(SessioniQrDTO sessioneQrDto)
        {
            try
            {
                ...
                await _repository.AddAsync(sessioneQrDto);

                // ✅ Audit trail
                LogAuditTrail("CREATE_QR_SESSION", "SessioniQr", sessioneQrDto.SessioneId.ToString());
                LogSecurityEvent("QRSessionCreated", new
                {
                    SessioneId = sessioneQrDto.SessioneId,
                    TavoloId = sessioneQrDto.TavoloId,
                    User = User.Identity?.Name
                });

                return CreatedAtAction(nameof(GetById),
                    new { id = sessioneQrDto.SessioneId },
                    sessioneQrDto);
            }
            catch (System.Exception ex)
            {
        ...
        public async Task<ActionResult> Update(Guid id, SessioniQrDTO sessioneQrDto)
        {
            try
            {
                ...
                if (id != sessioneQrDto.SessioneId)
                    return SafeBadRequest("ID sessione non corrispondente");
            ...
            {
                _logger.LogWarning(ex, "Sessione QR non trovata per aggiornamento: {Id}", id);
        ...
        [HttpGet("{id}/exists")]
        [AllowAnonymous] // ✅ UTILE PER VERIFICHE RAPIDE
        public async Task<ActionResult<bool>> Exists(Guid id)
        {
            try
            {
                if (id == Guid.Empty)
                    return SafeBadRequest<bool>("ID sessione non valido");
